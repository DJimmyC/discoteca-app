...
type ProductoMasVendidoItem = {
  ...
  descripcion?: string;
  marca?: string;
  cantidadVendida?: number | string;
  cantidad?: number | string;
  totalCantidad?: number | string;
  totalVendido?: number | string;
  total?: number | string;
  montoTotal?: number | string;
};

function obtenerListaProductosMasVendidos(
  reporte: ProductosMasVendidosResponse | undefined
): ProductoMasVendidoItem[] {
  const respuesta = reporte as any;
  ...
  if (Array.isArray(respuesta.productosMasVendidos)) {
    return respuesta.productosMasVendidos;
  }

  if (Array.isArray(respuesta.ranking)) {
  ...
  if (Array.isArray(respuesta.reporte)) {
...
export default function SucursalDetailsView() {
  ...
  const vendedorDestacado =
    dashboard?.vendedorMayorVenta as any;

  const idPerfilMeseroDestacado =
    useMemo(() => {
      return obtenerIdPerfil(
        vendedorDestacado
      );
  ...
  const {
    data: perfilMeseroDestacado,
    isLoading: cargandoPerfilMesero,
  } = useQuery<
    any,
    Error
  >({
  ...
  const productoDestacado =
    dashboard?.productoMasVendido as any;

  const nombreVendedor =
    cargandoPerfilMesero
      ? "Cargando mesero..."
      : obtenerNombrePerfil(
        perfilMeseroDestacado
      ) ||
      obtenerNombrePerfil(
        vendedorDestacado
      ) ||
            ...
            <article className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm xl:col-span-2">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Ranking resumido de los productos con mayor cantidad vendida.
                  </p>
                </div>
              ...
              {cargandoProductos ? (
                ...
                <div className="mt-5 grid gap-5 lg:grid-cols-[1.4fr_0.8fr]">
                  ...
                  <div className="h-[310px] rounded-xl border border-gray-100 bg-gray-50 p-3">
                    <ResponsiveContainer
                      width="100%"
                      height="100%"
                    >
                ...
                <div className="mt-5 flex h-[310px] flex-col items-center justify-center rounded-xl border border-dashed border-gray-300">
                  ...
                  <p className="mt-1 text-sm text-gray-400">
                    Cuando existan ventas pagadas, aquí aparecerá el ranking de productos.
                  </p>
                </div>
              )}
            </article>
